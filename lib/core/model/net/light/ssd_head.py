...
class SSDHead():
    ...
    def max_out_cla(self,fm,ratios_per_pixel,cls_num=2,scope='nul'):
        dim_h = tf.shape(fm)[1]
        dim_w = tf.shape(fm)[2]
        with tf.variable_scope(scope):
            ...
            with tf.variable_scope('positive'):
                pos_pre = slim.conv2d(fm, ratios_per_pixel*(cls_num-1), [3, 3], stride=1, activation_fn=None,
                                                normalizer_fn=None, scope='out_cla1')
                pos_pre = tf.reshape(pos_pre, ([-1, dim_h, dim_w, ratios_per_pixel, (cls_num-1)]))
